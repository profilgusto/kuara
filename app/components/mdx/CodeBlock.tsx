...
export default function CodeBlock({ code, className }: Props) {
    ...
    const lang = (className || '').match(/language-([A-Za-z0-9+-]+)/)?.[1] ?? undefined
    ...
    return (
        <div className="mx-auto my-6 max-w-3xl">
            <div className="relative rounded-xl border shadow-sm bg-zinc-50 dark:bg-zinc-900">
                ...
                <pre ref={preRef} className="overflow-x-auto p-4 pt-7 rounded-md bg-[#f6f8fa] dark:bg-[#0d1117]">
                    <code ref={codeRef} className={[className, 'hljs'].filter(Boolean).join(' ')}>
