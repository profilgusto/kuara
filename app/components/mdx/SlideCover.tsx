...
export default function SlideCover({
    ...
    return (
        <div className="absolute inset-0 z-50 flex flex-col justify-center items-center overflow-hidden rounded-2xl bg-background text-foreground m-0 p-0">
            ...
            {/* Content Layer */}
            <div className="relative z-20 w-full h-full flex flex-col items-center justify-center p-12 text-center" style={{ color: backgroundImage ? 'white' : 'inherit' }}>
                ...
                {logoImage && (
                    <div className="absolute top-8 right-8">
                        ...
                        <img src={logoImage} alt="Logo" className="max-h-16 w-auto object-contain" />
                    </div>
                )}
                ...
                {(author || date) && (
                    <div className="absolute bottom-8 left-0 right-0 flex flex-col items-center justify-end space-y-1">
                        {author && <p className="text-lg font-medium opacity-80 uppercase tracking-widest">{author}</p>}
                        {date && <p className="text-base opacity-75 tracking-wider">{date}</p>}
                    </div>
                )}
            </div>
        </div>
