...
export default function PDF({
  ...
  if (typeof title === "string" && title) {
    ...
    if (!mSm || !mLg) {
      const altFmt = title.match(
        /(?:^|\s)size=(\d{1,3})\s*,\s*(\d{1,3})(?=\s|$)/i,
      );
      if (altFmt) {
  ...
  if (typeof wsm === "number")
    (styleVars as Record<string, string>)["--mdx-pdf-wsm"] = `${wsm}vw`;
  if (typeof wlg === "number")
    (styleVars as Record<string, string>)["--mdx-pdf-wlg"] = `${wlg}vw`;
