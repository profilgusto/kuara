...
export default function PDFViewer({ url, title }: Props) {
  ...
  const zoomIn = () =>
    applyZoom(Math.min(MAX_ZOOM, +(visualScale + ZOOM_STEP).toFixed(2)));
  const zoomOut = () =>
    applyZoom(Math.max(MIN_ZOOM, +(visualScale - ZOOM_STEP).toFixed(2)));
  ...
  const pageWidth = containerWidth
    ? Math.round(containerWidth * renderScale)
    : undefined;
