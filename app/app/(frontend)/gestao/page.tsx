...
import { BookOpen, GraduationCap, Calendar, Users, ChevronRight } from 'lucide-react'
                ...
                {activeOffers.length === 0 ? (
                    <div className="text-center py-12 bg-muted/10 rounded-xl border border-dashed">
                        ...
                        <p className="text-muted-foreground text-sm">
                            ...
                            <Link href="/admin/collections/offers/create" className="underline text-primary">
                                Criar no admin
                            </Link>
                        </p>
                    </div>
...
function OfferCard({ offer, archived = false }: { offer: any; archived?: boolean }) {
    ...
    const studentCount = Array.isArray(offer.students) ? offer.students.length : 0

    return (
        <Link
            href={`/gestao/ofertas/${offer.id}`}
            className={`group block rounded-xl border p-5 transition-all hover:shadow-md hover:border-primary/30 ${archived ? 'opacity-60 hover:opacity-80' : ''
                }`}
        >
