...
export function GroupBuilderClient({
    ...
    function findStudent(id: string): { student: Student; source: string } | null {
        const fromUnassigned = unassigned.find((s) => s.id === id)
        if (fromUnassigned) return { student: fromUnassigned, source: 'unassigned' }
...
function DroppableZone({
    ...
    return (
        <div
            ref={setNodeRef}
            className={`min-h-[80px] rounded-xl border-2 border-dashed p-2 transition-colors ${isOver ? 'border-primary/50 bg-primary/5' : 'border-border'
                }`}
        >
            {children}
        </div>
