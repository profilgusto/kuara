...
import { getCurrentUser, getOffer, getOfferGroups } from '@/lib/payload-professor'
...
export default async function GroupsPage({
    ...
    const allStudents = Array.isArray(offer.students)
        ? offer.students.map((s: any) =>
            typeof s === 'object' ? { id: s.id, name: s.name || s.email || 'Aluno' } : { id: s, name: 'Aluno' }
        )
        : []
    ...
    const groupsData = groups.map((g: any) => {
        const members = Array.isArray(g.students)
            ? g.students.map((s: any) => {
                ...
                return { id: student.id, name: student.name || student.email || 'Aluno' }
            })
            : []
