...
import { getCurrentUser, getOffer, getOfferActivities, getOfferScores, getOfferGroups, getOfferPosts } from '@/lib/payload-professor'
import { ArrowLeft, BookOpen, ClipboardList, Megaphone, Calendar } from 'lucide-react'
...
export default async function StudentOfferPage({
    ...
    // Check enrollment
    const isEnrolled = Array.isArray(offer.students)
        && offer.students.some((s: any) =>
            (typeof s === 'object' ? s.id : s) === user.id
        )
    if (!isEnrolled && user.role === 'student') notFound()

    const courseTitle = typeof offer.course === 'object' ? offer.course.title : 'Disciplina'
    ...
    const studentGroup = groups.find((g: any) => {
        ...
        return members.some((s: any) => (typeof s === 'object' ? s.id : s) === user.id)
    })
    ...
    function getStudentScore(actId: string, actType: string): number | null {
        ...
        const individual: any = scores.find(
            (s: any) =>
                (typeof s.activity === 'object' ? s.activity.id : s.activity) === actId &&
                s.entityType === 'student' &&
                (typeof s.student === 'object' ? s.student?.id : s.student) === user!.id
        )
        ...
        if (actType === 'group' && studentGroup) {
            const groupScore: any = scores.find(
                (s: any) =>
                    (typeof s.activity === 'object' ? s.activity.id : s.activity) === actId &&
                    s.entityType === 'group' &&
                    (typeof s.group === 'object' ? s.group?.id : s.group) === studentGroup.id
            )
    ...
    const totalWeight = activities.reduce((s: number, a: any) => s + (a.weight || 0), 0)
    ...
    const currentModule = typeof offer.currentModule === 'object' ? offer.currentModule : null
            ...
            <header className="mb-8">
                ...
                {studentGroup && (
                    <p className="text-sm text-muted-foreground mt-1">
                        Grupo: <span className="font-medium text-foreground">{studentGroup.name}</span>
                    </p>
                )}
            </header>
            ...
            <div className="space-y-10">
                ...
                {currentModule && (
                    <section>
                        ...
                        <Link
                            href={courseSlug ? `/disciplinas/${courseSlug}/${currentModule.slug}` : '#'}
                            ...
                            <table className="w-full text-sm">
                                <thead>
                                    <tr className="border-b bg-muted/30">
                                        ...
                                        <th className="text-center px-3 py-2 font-medium">Nota (%)</th>
                                    </tr>
                                </thead>
                                <tbody>
                                        ...
                                        return (
                                            ...
                                            >
                                                <td className="px-3 py-2">
                                                    <span className="font-mono font-bold text-primary mr-2">{act.acronym}</span>
                                                    {act.description}
                                                </td>
                                                <td className="text-center px-3 py-2">{act.weight.toFixed(1)}</td>
                                                ...
                                                <td className="text-center px-3 py-2 font-medium">
                                                    {pct !== null ? `${pct}%` : <span className="text-muted-foreground/40">—</span>}
                                                </td>
                                            </tr>
                                        )
                                    })}
                                ...
                                <tfoot>
                                    <tr className="bg-primary/5 font-bold">
                                        ...
                                        <td className="text-center px-3 py-2">
                                            {finalScore !== null ? (
                                                <span className={finalScore >= 5 ? 'text-green-600 dark:text-green-400' : 'text-red-600 dark:text-red-400'}>
