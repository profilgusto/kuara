...
function extractLexicalMediaIds(lexicalData: unknown): Set<number | string> {
    ...
    function traverse(node: unknown): void {
        ...
        if (n.type === 'upload' && n.relationTo === 'media') {
            const value = n.value
            if (typeof value === 'string' || typeof value === 'number') ids.add(value)
        ...
        if ((n as Record<string, unknown>).root) traverse((n as Record<string, unknown>).root)
    }
...
export async function POST(request: NextRequest) {
    ...
    const usedInMap = new Map<number, Array<{ relationTo: 'modules' | 'posts'; value: number }>>()
    ...
    return NextResponse.json({ success: true, updated, total: allMedia.docs.length })
}
